...
export const pokemonCard = (pokemonList) => {
  ...
  for (const pokemon of pokemonList) {
      ...
      <figure class= "pokemon-container">
          <div class="pokemon-card">
            <div class="card-front">
              <h4 class="poke-name">${pokemon.name.charAt(0).toUpperCase() + pokemon.name.slice(1)}</h4>
              <h5 class= "poke-genera">${pokemon.genera}</h5>
              <img src=${pokemon.image}></img>
              <div class="circle"></div>
              <h5 class="poke-id"> #${pokemon.id} </h5>
              <h5 class= "poke-types">${pokemon.pkmType
                .join(" / ")
                .replace(/\b\w/g, (ch) => ch.toUpperCase())}</h5>
              <div class="poke-info">
                <h6 class= "poke weight">Weight: ${pokemon.weight}kg</h6>
                <h6 class= "poke-happiness"> Base Happiness: ${pokemon.baseHappiness}%</h6>
                <h6>Height: ${pokemon.height}m</h6>
            ...
            <div class="card-back">
              ...
              <div class="poke-stats-bar" style="background: linear-gradient(to right, ${
                colors[pokemon.pkmType[0]]
              } ${pokemon.attack}% , ${colors[pokemon.pkmType[0]]}70 ${pokemon.attack}%"
              ></div>
    ...
    if (pokemon.pkmType[1]) {
      pkmCard.style.background = `linear-gradient(145deg, ${
        colors[pokemon.pkmType[0]]
